/**
 * Agent Installation Script for Mnemosyne
 *
 ...
 *
 * Usage:
 *   npx ts-node install-agents.ts [path-to-data.json]
 *
 * The script will:
...
interface AgentConfig {
    ...
    createdAt: number;
    updatedAt: number;
    testStatus: 'never' | 'pending' | 'passed' | 'failed';
}

/**
 * Agent configurations based on Complete-Agent-Configuration-Guide.md
 */
function createAgentConfigs(defaultLlmId: string): AgentConfig[] {
    const now = Date.now();
...
Return JSON array of mitigation strategies:

\`\`\`json
[
  {
    "id": 1,
    "description": "Clear description of mitigation",
    "type": "Avoid|Reduce|Transfer|Accept",
    "effectivenessRating": "High|Medium|Low",
    "impactReduction": 0-5,
    "likelihoodReduction": 0-5,
    "estimatedCostK": number,
    "estimatedEffortHours": number,
    "implementationTimeline": "2 weeks",
    "pros": ["Benefit 1", "Benefit 2"],
    "cons": ["Drawback 1", "Drawback 2"],
    "recommendation": "Why this is a good/bad choice"
  }
]
\`\`\`
...
- Prioritize by effectiveness and feasibility`,
        ...
        // 7. Risk Assessment Agent
        {
            ...
            llmId: defaultLlmId,
            systemPrompt: `You are an expert Risk Assessment Agent specializing in the 5×5 Risk Matrix methodology (Likelihood × Impact = Risk Score).
...
Assess across dimensions: Schedule, Cost, Performance, Technical
Use the **HIGHEST** impact dimension for final score.
...
Return JSON with complete assessment including likelihood, impact breakdown by dimension, risk score, RAG band, exposure value, and detailed rationale.

# Guidelines

- Be evidence-based
- Use highest impact dimension
- Calculate exposure as sum of all cost impacts
...
- **Acceleration:** Is rate increasing?
...
- Current vs starting vs peak scores
- Velocity and momentum
- Likelihood and impact trends separately
- Mitigation effectiveness rating
- Actionable insights and recommendations
- Visual summary with emoji

# Guidelines

- Identify inflection points
- Correlate changes with mitigations
...
- Provide actionable recommendations`,
            ...
            allowDangerousOperations: false,
            folderScope: ['01_Risks'],
            capabilities: ['trend-analysis', 'risk-tracking', 'historical-analysis', 'pattern-recognition', 'data-visualization'],
            category: 'risk-management',
            visibility: 'public',
            temperature: 0.5,
            maxTokens: 2000,
        ...
        // 9. Program Risk Rollup
        {
            ...
            llmId: defaultLlmId,
            systemPrompt: `You are an expert Program Risk Rollup Agent specializing in aggregating risks across programs for executive reporting.
...
- Executive summary (metrics, insights, actions)
- Top 10 risks ranked by score
- Program breakdown (count, exposure, categories)
- Category analysis (distribution, trends)
- Trend summary (increasing/decreasing/stable)
- Mitigation summary (effectiveness, investment)
- Priority-ranked recommendations

# Guidelines

- Be quantitative (numbers, percentages)
- Highlight outliers and patterns
- Action-oriented recommendations
- Executive-friendly language
- Clear, concise, decision-focused`,
            retrievalSettings: {
                topK: 50,
                scoreThreshold: 0.3,
                searchStrategy: 'hybrid'
            ...
            capabilities: ['program-analysis', 'risk-aggregation', 'executive-reporting', 'portfolio-management', 'top-risk-identification'],
            category: 'risk-management',
            visibility: 'public',
            temperature: 0.4,
            maxTokens: 3000,
        ...
        // 10. Mitigation ROI Calculator
        {
            ...
            llmId: defaultLlmId,
            systemPrompt: `You are an expert Mitigation ROI Calculator specializing in cost-benefit analysis for risk mitigation strategies.

# Your Role

Calculate ROI for mitigation options to help prioritize spending:

1. **Calculate Risk Exposure** (before mitigation)
2. **Calculate Reduced Exposure** (after mitigation)
3. **Calculate Risk Reduction Value**
4. **Calculate Mitigation Cost**
5. **Calculate ROI** (Risk Reduction / Cost)
6. **Recommend Priority** (High/Medium/Low)
...
|-----|----------|----------------|
| >5.0 | High | Excellent, implement immediately |
| 2.0-5.0 | High | Strong ROI, prioritize |
| 1.0-2.0 | Medium | Positive return, fund if able |
| 0.5-1.0 | Medium | Marginal, consider alternatives |
| <0.5 | Low | Poor ROI, reconsider |
...
- Baseline risk (current state)
- Per-mitigation analysis (cost, reduction, ROI)
- Comparison (best ROI, most effective, lowest cost)
- Recommendations (primary, secondary, combined approach)

# Calculation Details

- Labor Cost: Assume $100/hour fully burdened
- Exposure: (Likelihood/5) × (Impact/5) × Contract Value
- Payback: (Cost / Reduction) × 12 months

# Guidelines

- Consider all costs (implementation + labor + ongoing)
 ...
 */
function installAgents(dataPath?: string): void {
    ...
    if (!fs.existsSync(targetPath)) {
        ...
        console.error('   Usage: npx ts-node install-agents.ts /path/to/.obsidian/plugins/mnemosyne/data.json');
        process.exit(1);
    }
    ...
    // Get default LLM
    const defaultLlm = settings.llmConfigs.find((llm: any) => llm.enabled) || settings.llmConfigs[0];
    if (!defaultLlm) {
    ...
    const existingIds = new Set(settings.agents.map((a: any) => a.id));
